    ...
    updateOne: function (req, res) {
        ...
        try{
            MongoClient.connect(url, {useUnifiedTopology: true}, function(err, db) {
                ...
                return dbo.collection(auth[4][1]).updateOne(auth[5][1], {$set:auth[6][1]}, function(err, obj) {
                    if (err)  return  res.send('{"status":"error", "desc":'+ err + '}');
                    db.close();
                    return res.send('{"status":"ok", "desc":"' + obj.result.n + ' documents(s) updated"}');
                });
            });
        } catch (e) {
    ...
    updateMany: function (req, res) {
        ...
        try{
            MongoClient.connect(url, {useUnifiedTopology: true}, function(err, db) {
                if (err) return  res.send('{"status":"error", "desc":'+ err + '}');
                var dbo = db.db(auth[3][1]);
                return dbo.collection(auth[4][1]).updateMany(auth[5][1], {$set:auth[6][1]}, function(err, obj) {
                    if (err)  return  res.send('{"status":"error", "desc":'+ err + '}');
                    db.close();
                    return res.send('{"status":"ok", "desc":"' + obj.result.n + ' documents(s) updated"}');
                });
